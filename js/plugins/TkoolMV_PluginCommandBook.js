 ...
 * ===========================================================================
 * レベルの変更
 *  アクターのレベルを変更します。増減ではなく変更后のレベルを指定できます。
 *  製作者 Alec
 *
 *  パラメータ
 *  　アクターのID（もし0なら全員）
 ...
 *  引数2：対象の文字列
 *  引数3：開始位置
 *  引数4：終了位置(End か 最后 の指定で最后まで切り取り)
 *  引数5：開始方向(省略可)[前方(省略値)  后方]
 *                                 or
 *                         [front(省略値) back]
 * 使用例
 *  文字の切取 1 \JS[$dataMap.note]\JS 1 10 前方
 *  文字の切取 1 \JS[$dataMap.note]\JS 10 最后 后方
 ...
 *  更新されるセーブファイルは、最后にセーブ or ロードしたファイルになります。
...
(function(){
    ...
    Game_Interpreter.prototype.pluginCommandBook_文字の置換 = function(args) {
        ...
        args[4]=args[4].replace(/back/i,'后方');
    ...
    Game_Interpreter.prototype.pluginCommandBook_文字の切取 = function(args) {
        ...
        var Nm2 = args[3]
        if (Nm2.toLowerCase() == 'end' || Nm2 == '最后' ) Nm2 = Str.length;
    ...
    Game_Interpreter.prototype.pluginCommandBook_文字の検索 = function(args) {
        ...
        args[3]=args[3].replace(/back/i,'后方');
        switch (args[3]){
          case '前方':
          default:
