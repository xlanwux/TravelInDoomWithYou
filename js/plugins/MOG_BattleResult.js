...
// ** get Exp Rate
//==============================
BattleManager.getExpRate = function() {
	return $gameActors.actor(1).equips().filter(e => e)
                                        .reduce((a, e) => a.concat(e.traits), [])
                                        .filter(t => t.code == 23 && t.dataId == 9)
                                        .map(t => t.value)
                                        .reduce((a, v) => a * v, 1);
};
...
BattleManager.getResultData = function() {
	...
	//  $gameTemp._bResult[1] = this._rewards.exp;
	 $gameTemp._bResult[1] = Math.ceil(this._rewards.exp * this.getExpRate());
	 $gameTemp._bResult[2] = this._rewards.gold;
	 $gameTemp._bResult[3] = this._rewards.items;
	 $gameTemp._bResult[4] = []; 
	 $gameTemp._bResult[5] = this._rewards.exp;
};
...
BattleResult.prototype.updateVictAnimation = function() {
	   if (this._phaseAnime[0] === 1) { 
		    if (this._vict.scale.x > 1.00) {
				this._vict.scale.x -= 0.1;
				this._vict.opacity += 30;
				if (this._vict.scale.x <= 1.00) {
...
BattleResult.prototype.updateLevel = function() {
	...
	if (!this._actor && this._actorIndex >= 1) {
		this._phase = 5;
		SoundManager.playCursor();
	} else {
		if (this._actor) {
...
BattleResult.prototype.gainEXP = function() {
    ...
    //  actor.gainExp($gameTemp._bResult[1]);
     actor.gainExp($gameTemp._bResult[5]);
     this._actorIndex += 1;
