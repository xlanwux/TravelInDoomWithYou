 ...
 *
 * ・スキル使用后
 * スキルを使用して、相手に効果が適用された后で、副作用が適用されます。
 ...
 * <SES_スキル使用后:1,5>  # スキル使用后、効果[1][5]を使用者に適用（[,]区切り）
...
(function() {
    ...
    DataManager.categorizeItemEffects = function(data) {
        ...
        var numbersOnAfter  = this.getSideEffectNumbers(data, ['OnSkillAfter', 'スキル使用后']);
